...
def pegaCodigo(cutter):
    ...
    if(len(autor_input.get()) > 1):
        nome = terminal.FormataNome(autor_input.get()).split(',')[0]
        codigo = terminal.CodigoAutor(nome, cutter)
...
def livroCodigo(codigo):
    ...
    if(len(titulo) > 1):
        codigo = terminal.CodigoLivro(codigo, titulo)
...
def revisarFunc(cutter):
    ...
    if(len(autor_input.get()) >= 1):
        nome = terminal.FormataNome(autor_input.get())
        g_autor = nome
...
def enviarFunc(driver, cutter):
    ...
    terminal.novo_registro_func(nome, titul, codigo, classif, exemp, volume_n, driver)
...
def login_bib(driver):
    ...
    bibliografia = terminal.login(user, senha, driver)
...
def NovoExemplar(driver, index, pesquisa):
    ...
    terminal.NovoExemplar(driver, index, pesquisa)
...
def Existentes(driver):
    ...
    driver.get(bibliografia)    
    number = terminal.ResultsSearch(nome, driver)
    ...
    if(nome != '' and number != 0):
        ...
        for i in range(1, number + 1):
            buttons_frame_labels.append(CTkLabel(buttons_frame_item[0], text=terminal.GetTextIndex(driver, i), font=CTkFont("Calibri", 14, weight='bold'), text_color='#1f1f1f'))
...
cutter = terminal.inicia_cutter()
driver = terminal.inicia_driver()
